...
class User(UserMixin):
    def __init__(self, user_id, username, password, profile_picture, firstName='', lastName='', bio='', location=''):
        self.id = user_id
...
def load_user(user_id):
    ...
    if user_data:
        return User(user_data['id'], user_data['username'], user_data['password'], 
                   user_data['profile_picture'], user_data['firstName'], user_data['lastName'],
                   user_data['bio'], user_data['location'])
    return None
...
def profile(username):  # Changed parameter
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM user WHERE username = ?', (username,))  # Changed query
    user_data = cursor.fetchone()
    conn.close()

    if user_data:
        profile_user = User(user_data['id'], user_data['username'], user_data['password'], 
                    user_data['profile_picture'], user_data['firstName'], user_data['lastName'],
                    user_data['bio'], user_data['location'])
...
def create_post():
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('profile', username=current_user.username))  # Changed to username
...
def edit_profile():
    if request.method == 'POST':
        ...
        if changeProfilePicture:
            ...
            changeProfilePicture.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            conn = sqlite3.connect(DATABASE_NAME)
            cursor = conn.cursor()
            cursor.execute('UPDATE user SET profile_picture = ? WHERE id = ?', (f"{filename}", current_user.id))
            conn.commit()
            conn.close()
        ...
        if not is_valid_username(changeUsername):
            flash('Username can only contain letters and numbers without spaces')
            return render_template('edit_profile.html', user=current_user, UPLOAD_FOLDER=UPLOAD_FOLDER)

        update_profile(current_user.id, changeUsername, firstName, lastName, bio, location)
        flash('Profile updated successfully!')
        return redirect(url_for('profile', username=changeUsername))  # Changed to username
...
def feed():
    try:
        ...
        posts = get_posts()
        return render_template('feed.html', posts=posts, UPLOAD_FOLDER=UPLOAD_FOLDER, current_user=current_user)
